...
@section('content')
    <div class="barangay-container"> 
        <!-- Update Barangay View -->
        <h1>Update Barangay</h1>

        <!-- Form for updating an existing barangay -->
        <form method="POST" action="{{ route('barangays.update', $barangay) }}">
            @csrf
            @method('PUT') <!-- Specifying the HTTP method as PUT -->
            ...
            <div class="barangay-form-group">
                ...
                <select id="city_id" name="city_id" class="barangay-select">
                    @foreach ($cities as $city)
                        <!-- Options for each city with current city selected -->
